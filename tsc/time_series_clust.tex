...
%         \midrule
%         Bayesian information criterion (BIC). & \cite{mixture_gaussian_hmm, } \\
%         Duda-Hart index.                      & \cite{financial_tsc_variance_ratio, } \\
%         L-curve.                              & \cite{hier_clust_w_state_space_models, } \\
%         Gap statistic.                        & \cite{hier_clust_w_state_space_models, temporal_tsc_threshold_ar_models} \\
%         Accuracy.                             & \cite{clust_large_datasets_aghabozorg, } \\
%         Precision.                            & \cite{topology_for_shape_based_tsc, clust_large_datasets_aghabozorg, wavelet_multivar_tsc_multi_pca, fuzzy_c_means_pso_svd, multivariate_tsc_common_pca} \\
%         Recall.                               & \cite{topology_for_shape_based_tsc, clust_large_datasets_aghabozorg, wavelet_multivar_tsc_multi_pca} \\
% 		F1-score							  & \cite{topology_for_shape_based_tsc, } \\
%         Matthews correlation coefficient.     & \cite{topology_for_shape_based_tsc, } \\
%         Rand index.                           & \cite{community_detection_networks_tsc, garch_robust_tsc, BSLEX_nonlin_nonstat_tsc, copula_fuzzy_tsc_spatial, apxdist_sax_k_modesi, multivar_tsc_community_detection, hysteresis_tsc_tensor_decomp, } \\
%         Adjusted Rand index.                  & \cite{copula_fuzzy_tsc_spatial, multivar_tsc_community_detection, hysteresis_tsc_tensor_decomp, auto_encoder_many_tsc_algorithms, fstar_hac_tsc, fstar_hac_tsc} \\
%         F-measure.                            & \cite{clust_large_datasets_aghabozorg, apxdist_sax_k_modes, fuzzy_c_means_pso_svd, xml_dft_delaunay_traingulation, } \\
%         Xie-Beni index.                       & \cite{garch_robust_tsc} \\
%         Silhouette index.                     & \cite{BSLEX_nonlin_nonstat_tsc, multivariate_tsc_hmm, copula_fuzzy_tsc_spatial, tsc_ar_metric_air_pollution, wavelet_multivar_tsc_multi_pca, stock_price_tsc_regr_trees_som, copula_ica_tsc, load_tsc_state_space_model} \\
%         Entropy.                              & \cite{BSLEX_nonlin_nonstat_tsc, apxdist_sax_k_modes, } \\
%         Purity.                               & \cite{BSLEX_nonlin_nonstat_tsc, apxdist_sax_k_modes, multivar_tsc_community_detection, xml_dft_delaunay_traingulation, var_multivar_tsc} \\
%         Bavies-Bouldin index.                 & \cite{multivariate_tsc_hmm, } \\
%         Dunn index.                           & \cite{multivariate_tsc_hmm, tsc_total_variation_distance, copula_ica_tsc} \\
%         RMSE.                                 & \cite{shape_feat_mod_tsc_rfa} \\
%         Relative RMSE.                        & \cite{shape_feat_mod_tsc_rfa} \\
%         SSE.                                  & \cite{apxdist_sax_k_modes} \\
%         Nash criterion.                       & \cite{shape_feat_mod_tsc_rfa} \\
%         Normalized Mutual Information (NMI).  & \cite{apxdist_sax_k_modes, tensor_multi_elastic_kernel_tsc} \\
%         Jaccard coefficient.                  & \cite{apxdist_sax_k_modes, hysteresis_tsc_tensor_decomp, } \\
%         Folks and Mallows (FM).               & \cite{apxdist_sax_k_modes, hysteresis_tsc_tensor_decomp, } \\
%         CH index.                             & \cite{copula_ica_tsc} \\
%         C-index.                              & \cite{copula_ica_tsc} \\
%         Friedman index.                       & \cite{copula_ica_tsc} \\
%         SDbw index.                           & \cite{copula_ica_tsc} \\
%         Misclassification rates.			  & \cite{comp_many_model_based_tsc_GMM} \\
%         \bottomrule
%     \end{tabular}
%     \caption{Evaluation indices}
%     \label{tab:eval_ind}
% \end{table*}
